...
def verificar_caminho(caminho_arquivo):
    ...
    if not os.path.exists(caminho_arquivo):
        print(f'O caminho {caminho_arquivo} não existe.')
        return False
    return True
...
def extrair_sentencas(body):
    ...
    if body is not None and body.text:
        ...
        if ((wrong is not None) and (correct is not None)):
            labeled_sentences.append(f'{wrong.text}|{correct.text}')

    return labeled_sentences


def percorrer_pastas(caminho_arquivo):
    # Percorre as pastas e arquivos XML no caminho base e extrai sentenças
    labeled_sentences_total = []

    for root_dir, _, files in os.walk(caminho_arquivo):
        for file in files:
...
def main():
    ...
    arquivo_saida = input(
        "Digite o nome do arquivo de saída (ex: resultado.txt): ")
