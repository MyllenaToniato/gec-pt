...
def varrer_arquivos_sentencas_sem_erro(diretorio_base, arquivo_saida):
    """Gera um TSV contendo frases sem marcação de erro (<wrong> ou <correct>)"""
    dados = []
    ...
    for raiz, _, arquivos in os.walk(diretorio_base):
        for arquivo in arquivos:
            if arquivo.endswith('.xml') and not arquivo.startswith('.') and arquivo != "prompt.xml":
                caminho_arquivo = os.path.join(raiz, arquivo)
                body_text = ler_arquivo_xml(caminho_arquivo)
                frases_sem_erros = extrair_sentencas_sem_erros(body_text)

                for frase in frases_sem_erros:
                    dados.append([frase.strip(), arquivo, raiz])
    ...
    print(f"Arquivo TSV '{arquivo_saida}' com frases sem erro criado com sucesso!")
...
CSV_SAIDA_SEM_ERRO = "resultado_sentencas_sem_erros.tsv"
varrer_arquivos_sentencas_sem_erro(DIRETORIO_RAIZ, CSV_SAIDA_SEM_ERRO)
